...
class Weight_GL(QtOpenGL.QGLWidget):
    ...
    def update_weight_bar(self):
        ...
        self.draw_bar(1.0, 0.05, -0.5, 0.0, 255.0, 255.0, 255.0)
        self.draw_bar(1.0, 0.05,  0.5, 0.0, 255.0, 255.0, 255.0)


        if  abs(self.normalized_weight < 0.45):
            self.draw_bar(0.5, self.normalized_weight, self.x_pos, self.y_pos,   0.0, 255.0, 0.0)
        else:
            self.draw_bar(0.5, self.normalized_weight, self.x_pos, self.y_pos, 255.0,  50.0, 0.0)
    ...
    def draw_bar(self, height, width, x_pos, y_pos, r, g, b):
        ...
        gl.glVertex2f(width + x_pos,-height/2 + y_pos)
        gl.glVertex2f(width + x_pos, height/2 + y_pos)
        gl.glVertex2f( 0.0  + x_pos, height/2 + y_pos)
        gl.glVertex2f( 0.0  + x_pos,-height/2 + y_pos)
        gl.glEnd()
