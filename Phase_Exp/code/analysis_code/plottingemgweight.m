...
for i=1:size(cop4,1)
   for j=1:length(cop4)
copf7(i,j)=p0_cop_torque(i,j)*100/weight7(i,j);
copf4(i,j)=p7_cop_torque(i,j)*100/weight4(i,j);
   end
end
...
copf_20s=t(2)*std(copf_20,'omitNaN')/sqrt(size(copf4,1));
copf_40s=t(2)*std(copf_40,'omitNaN')/sqrt(size(copf4,1));
...
for i=1:size(copf7)
if(abs(copfh_20(i))>20)
    copfh_20(i)=NaN;
end
if(abs(copfh_40(i))>20)
    copfh_40(i)=NaN;
end

if(abs(copfh_60(i))>20)
    copfh_60(i)=NaN;
end
...
copfh_20s=t(2)*std(copfh_20,'omitNaN')/sqrt(size(copf7,1));
copfh_40s=t(2)*std(copfh_40,'omitNaN')/sqrt(size(copf7,1));
copfh_60s=t(2)*std(copfh_60,'omitNaN')/sqrt(size(copf7,1));
figure
plot(time,copf4m,'b');
hold on
plot(time,copf7m,'r');
...
figure
plot(time,p0_foot_posm*180/pi,'b');
hold on
plot(time,p7_foot_posm*180/pi,'r');
xline(150,'-','20%')
xline(300,'-','40%')
xline(450,'-','60%')
formatSpec = 'Foot angle Profile %s';
str = sprintf(formatSpec,sub_name);
title(str)
legend('Rigid','Haptic');
xlabel('time(ms)');
ylabel('Angle(deg)');
saveas(gcf,'footangle_plot.jpg');
saveas(gcf,'footangle_plot.fig');
